...
func main() {
	...
	paths := network.findAll(startPath, false)
...
func (n *network) findAll(visited []*cave, smallTwice bool) [][]*cave {
	paths := make([][]*cave, 0)
	...
	for _, next := range currCave.connections {
		...
		} else if next.small && !smallTwice && contains(visited, next) {
			newVisited := append(visited, next)
			paths = append(paths, n.findAll(newVisited, true)...)
		} else if !next.small || !contains(visited, next) {
			newVisited := append(visited, next)
			paths = append(paths, n.findAll(newVisited, smallTwice)...)
		}
	}
